...
let TOOLS = ['pencil', 'eraser', 'eraseArea', 'text', 'image', 'undo', 'trash', 'pan', 'lineStraight', 'rectangle', 'rectangleFilled', 'ellipse', 'ellipseFilled', 'marker', 'select', 'postit', 'presentation'];
...
/*aww.onReceiveOps((ops) => {
    ...
    ops.forEach((op) => {
        if (op.op === 'chat') {
            handleMessages(op.text, op.uid, op.time);
        }
    });
});
